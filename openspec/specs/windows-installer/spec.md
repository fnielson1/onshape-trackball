...
`setup.cmd`: the Windows batch installer mirroring `setup.sh`'s full surface —
prerequisite and driver install, the reboot gate, mouse selection, Scheduled Task
lifecycle, config creation and drift detection, the status board, reconfigure and
uninstall.
