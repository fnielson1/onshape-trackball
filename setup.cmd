...
rem  schtasks cannot express restart-on-failure, so it is patched in afterwards.
powershell -NoProfile -Command ^
  "$s = New-ScheduledTaskSettingsSet -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1) -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -ExecutionTimeLimit ([TimeSpan]::Zero); Set-ScheduledTask -TaskName '%TASK_NAME%' -Settings $s" >nul 2>&1
...
rem  Both halves are checked - the task must point at our launcher, and the launcher
rem  must still name this repo's gate.py.
schtasks /Query /TN "%TASK_NAME%" /XML 2>nul | find /i "%LAUNCHER%" >nul 2>&1
if errorlevel 1 exit /b 1
if not exist "%LAUNCHER%" exit /b 1
find /i "%GATE%" "%LAUNCHER%" >nul 2>&1
exit /b %errorlevel%
